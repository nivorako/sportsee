...
export const getUserData = (userId) => {
    const selected = USER_MAIN_DATA.find(
...
export const getUserActivity = (userId) => {
    const userActivity = [];
    const selected = USER_ACTIVITY.find(
...
export const getAverageSession = (userId) => {
    ...
    const selected = USER_AVERAGE_SESSIONS.find(
        (user) => user.userId === parseInt(userId)
    );
    for (let index in selected.sessions) {
        averageSession[index].sessionLength =
            selected.sessions[index].sessionLength;
    }
 ...
 *
 * @param {string} userId
 * @returns {{kind: string, value: number}[]} user's performance of the week according to kind
 */
export const getUserPerformance = (userId) => {
    ...
    const selected = USER_PERFORMANCE.find(
        (user) => user.userId === parseInt(userId)
    );
